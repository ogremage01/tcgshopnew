...
export default async function LorcSetPage({
    params,
    searchParams,
}: {
    ...
    const [setData, setBanner] = await Promise.all([
        serverApi.get<TcgPSetInfoDto>(`/api/game/lorc/sets/${encodedSet}`).catch(() => null),
        fetchSetBanner("lorc", set),
    ]);
        ...
        <div className="flex flex-col gap-4">
            <div className="flex flex-col gap-4">
                ...
                <h1 className="text-2xl font-bold">
                    {setData?.setName} {t("game.setList.sets")}
                </h1>
                <p className="text-sm text-muted-foreground">
                    {t("game.setList.code")}: {setData?.setCode}
                </p>
                <p className="text-sm text-muted-foreground">
                    {t("game.setList.releaseDate")}:{" "}
                    {new Date(setData?.releaseDate ?? "").toLocaleDateString()}
                </p>
            </div>
            <ProductBrowseSection
                searchParams={resolved}
                productsEndpoint={`/api/products/search/game/lorc/${setData?.setCode ?? ""}`}
                defaultSetCode={setData?.setCode}
                showFoilFilter
                headerContent={
                    <h1 className="text-2xl font-bold">
                        {setData?.setName} {t("game.setList.productsList")}
                    </h1>
                }
                emptyContent={
