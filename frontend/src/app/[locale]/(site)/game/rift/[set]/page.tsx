...
export default async function RiftSetPage({
    params,
    searchParams,
}: {
    ...
    const [setData, setBanner] = await Promise.all([
        serverApi.get<TcgPSetInfoDto>(`/api/game/rift/sets/${encodedSet}`),
        fetchSetBanner("rift", set),
    ]);
    ...
    return (
        <div className="flex flex-col gap-4">
            ...
            <ProductBrowseSection
                ...
                productsEndpoint={`/api/products/search/game/rift/${encodedSet}`}
                defaultSetCode={setData?.setCode ?? set}
                showFoilFilter
                headerContent={
                    <h1 className="text-2xl font-bold">
