...
import {
  ...
  AdminSingleCardSalesSummaryDto,
...
type ProductType = "single" | "sealed" | "supply" | "manual";
...
function SingleCardGameTable({
  gameSummary,
}: {
  gameSummary: AdminSingleCardSalesSummaryDto;
}) {
  return (
    <div className="flex flex-col gap-2">
      ...
      <table className="w-full border-collapse border border-black table-fixed">
        ...
        <tbody>
          {gameSummary.cardProductSaleDtoList.length > 0 ? (
            gameSummary.cardProductSaleDtoList.map((item) => (
              <tr key={item.setName}>
...
export function WeeklyOnlineDetailSection({ report }: Props) {
  const online = report.onlineSalesSummaryDto;

  const [productType, setProductType] = useState<ProductType | "">("");
  const [detailValue, setDetailValue] = useState("");
  const [query, setQuery] = useState<QueryState | null>(null);

  const singleList = online?.singleCardSalesSummaryList ?? [];
  const sealedList = online?.sealedProductSalesSummaryList ?? [];
  const supplyList =
    online?.supplyProductSalesSummaryDto?.supplyProductSaleDtoList ?? [];
  const manualList =
    online?.manualProductSalesSummaryDto?.manualProductSaleDtoList ?? [];

  const detailOptions: { value: string; label: string }[] = (() => {
    if (productType === "single") {
      return [
        { value: ALL_VALUE, label: "전체" },
        ...singleList.map((item) => ({ value: item.game, label: item.game })),
      ];
    }
  ...
  const renderResult = () => {
    ...
    if (query.productType === "single") {
      const summaries =
        query.detailValue === ALL_VALUE
          ? singleList
          : singleList.filter((item) => item.game === query.detailValue);
      ...
      return (
        <div className="flex flex-col gap-4">
          <h2 className="text-center text-lg font-bold">싱글 카드</h2>
    ...
    if (query.productType === "supply") {
      const rows =
        query.detailValue === ALL_VALUE
          ...
          : toSupplyRows(
              supplyList.filter(
                (item) => item.supplyType === query.detailValue,
              ),
            );
  ...
  return (
    <>
      <Separator />
      <h1 className="text-center text-lg font-bold">온라인 매출 세부</h1>
      <Separator />
      ...
      <div className="flex flex-row flex-wrap items-center justify-center gap-2">
        ...
        >
          ...
          <SelectContent>
            <SelectItem value="single">싱글 카드</SelectItem>
